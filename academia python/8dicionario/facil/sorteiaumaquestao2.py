...
def sorteia_questao(dicionario, questao):
    if questao == "facil":
        for niveis in dicionario:
            questoes = dicionario[niveis]
            for opcoes in range(len(questoes)):
                 i = random.radint(opcoes)
                 aleatorio = questoes[i]
    return aleatorio
dicionario = {"lara": []}
...
print(sorteia_questao(questoes, "facil"))
